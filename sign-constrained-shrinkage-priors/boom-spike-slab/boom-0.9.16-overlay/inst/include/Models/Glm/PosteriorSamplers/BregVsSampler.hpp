...
namespace BOOM {
  ...
  class BregVsSampler : public PosteriorSampler {
   public:
    ...
    // Indices arrive from R as one-based positions in the full coefficient
    // vector. They are converted to zero-based positions by the implementation.
    void set_sign_constraints(const std::vector<int> &positive,
   ...
   private:
    ...
    std::vector<int> positive_constraints_;
    std::vector<int> negative_constraints_;
