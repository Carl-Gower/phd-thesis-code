...
// Sign-constraint extension by Carl Gower, 2025.
/*
...
namespace BOOM {
  ...
  void BVS::set_sign_constraints(const std::vector<int> &positive,
                                 const std::vector<int> &negative) {
    positive_constraints_.clear();
    negative_constraints_.clear();
    for (int index : positive) {
      if (index > 0) positive_constraints_.push_back(index - 1);
    }
    for (int index : negative) {
      if (index > 0) negative_constraints_.push_back(index - 1);
    }
  }
  ...
  void BVS::draw_beta() {
    ...
    if (ok && positive_constraints_.empty() &&
        negative_constraints_.empty()) {
      ...
      const Selector &included = model_->coef().inc();
      const int number_included = posterior_mean_.size();
      std::vector<bool> is_positive(number_included, false);
      std::vector<bool> is_negative(number_included, false);

      int included_position = 0;
      for (int full_position = 0;
           full_position < model_->nvars_possible(); ++full_position) {
        if (included[full_position]) {
          is_positive[included_position] =
              std::find(positive_constraints_.begin(),
                        positive_constraints_.end(), full_position) !=
              positive_constraints_.end();
          is_negative[included_position] =
              std::find(negative_constraints_.begin(),
                        negative_constraints_.end(), full_position) !=
              negative_constraints_.end();
          ++included_position;
        }
      }

      Vector beta = model_->included_coefficients();
      if (beta.size() != number_included) beta = posterior_mean_;
      for (int j = 0; j < number_included; ++j) {
        const double conditional_variance = 1.0 / posterior_precision(j, j);
        double weighted_remainder = 0.0;
        for (int k = 0; k < number_included; ++k) {
          if (k != j) {
            weighted_remainder += posterior_precision(j, k) *
                (beta[k] - posterior_mean_[k]);
          }
        }
        const double conditional_mean =
            posterior_mean_[j] - conditional_variance * weighted_remainder;
        const double conditional_sd = std::sqrt(conditional_variance);

        if (is_positive[j]) {
          beta[j] = rtrun_norm_mt(
              rng(), conditional_mean, conditional_sd, 0.0, true);
        } else if (is_negative[j]) {
          beta[j] = rtrun_norm_mt(
              rng(), conditional_mean, conditional_sd, 0.0, false);
        } else {
          beta[j] = rnorm_mt(rng(), conditional_mean, conditional_sd);
        }
      }
      posterior_mean_ = beta;
      ...
      // could not be completed, so skip this draw and try again.
