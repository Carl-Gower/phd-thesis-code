...
lm.spike <- function(formula,
  ...
  ##   pos_con: One-based model-matrix column indices constrained positive.
  ##   neg_con: One-based model-matrix column indices constrained negative.
  ...
  check.constraint.indices <- function(index, argument) {
    if (is.null(index) || length(index) == 0L) return(integer(0))
    ...
    if (any(index < 1L | index > ncol(x))) {
      stop(argument, " indices must be between 1 and ", ncol(x), ".")
    }
    index
  }
  pos_con <- check.constraint.indices(pos_con, "pos_con")
  neg_con <- check.constraint.indices(neg_con, "neg_con")
  if (length(intersect(pos_con, neg_con))) {
    stop("pos_con and neg_con must not overlap.")
  }

  constraints.requested <- length(pos_con) || length(neg_con)
  if (constraints.requested && error.distribution != "gaussian") {
    stop("Sign constraints are currently supported only for Gaussian errors.")
  }
  if (constraints.requested && bma.method != "SSVS") {
    stop("Sign constraints are not implemented for the ODA sampler.")
  }
  if (constraints.requested && ncol(x) > model.options$adaptive.cutoff) {
    stop(paste0(
      "Sign constraints require the classic SSVS sampler; set ",
      "SsvsOptions(adaptive.cutoff = Inf)."
    ))
  }
  prior$pos_con <- pos_con
  prior$neg_con <- neg_con
