...
namespace {
  ...
  std::vector<int> IntVectorFromListElement(SEXP list, const char *name) {
    std::vector<int> result;
    SEXP element = getListElement(list, name);
    if (element == R_NilValue || Rf_isNull(element) ||
        !Rf_isInteger(element)) {
      return result;
    }
    result.reserve(LENGTH(element));
    for (int i = 0; i < LENGTH(element); ++i) {
      result.push_back(INTEGER(element)[i]);
    }
    return result;
  }
  ...
  Ptr<GlmModel> SpecifyRegressionModel(
    ...
    std::vector<int> positive =
        IntVectorFromListElement(r_spike_slab_prior, "pos_con");
    std::vector<int> negative =
        IntVectorFromListElement(r_spike_slab_prior, "neg_con");
    bool constraints_requested = !positive.empty() || !negative.empty();
    ...
    if (Rf_inherits(r_model_options, "SsvsOptions")) {
      ...
      if (model->xdim() <= adaptive_cutoff ) {
        ...
        NEW(BregVsSampler, ssvs_sampler)(
            ...
            prior.siginv_prior(),
            prior.spike());
        ssvs_sampler->set_sign_constraints(positive, negative);
        ...
        if (constraints_requested) {
          report_error(
              "Sign constraints are not implemented for the adaptive sampler.");
        }
      ...
      if (constraints_requested) {
        report_error("Sign constraints are not implemented for the ODA sampler.");
      }
