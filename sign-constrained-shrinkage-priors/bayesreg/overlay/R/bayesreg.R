# Derived from bayesreg 1.3 by Daniel F. Schmidt and Enes Makalic.
# Sign-constraint modifications by Carl Gower, 2025.
...
#' @param pos_con Integer indices of regression coefficients constrained to be
#'   non-negative. The intercept is not included in this indexing.
#' @param neg_con Integer indices of regression coefficients constrained to be
#'   non-positive. The intercept is not included in this indexing.
#' @section Details:
...
#'
#'
bayesreg <- function(formula, data,
                     model='normal', prior='ridge',
                     n.samples = 1e3, burnin = 1e3, thin = 5,
                     t.dof = 5, n.cores = Inf,
                     pos_con = NULL, neg_con = NULL)
{
  ...
  # Validate optional sign constraints. These indices refer to the columns of
  # X after the intercept has been removed.
  ...
  if ((!is.null(pos_con) || !is.null(neg_con)) && !SMN) {
    stop("Sign constraints are not implemented for the Metropolis-Hastings samplers used by poisson, geometric, or binomialmh models.")
  }
  ...
  n.available.cores = if (is.na(detected.cores)) 1L else max(1L, detected.cores - 1L)
  ...
  if (n.cores > 1)
  {
    ...
    rv.final = foreach::foreach(i=1:n.cores) %dopar%
      {
        rv.samples = bayesreg.sample.chain(X, y, z, p, n,
                                           n.samples.per.core, burnin, thin,
                                           mvnrue, groups, XtX, Xty,
                                           model, prior, t.dof,
                                           PrecomputedXtX, rv.gd, SMN, MH,
                                           pos_con = pos_con,
                                           neg_con = neg_con)
        rv.samples
      }
  ...
  {
    rv.final = list()
    rv.final[[1]] = bayesreg.sample.chain(X, y, z, p, n,
                                          n.samples.per.core, burnin, thin,
                                          mvnrue, groups, XtX, Xty,
                                          model, prior, t.dof,
                                          PrecomputedXtX, rv.gd, SMN, MH,
...
bayesreg.sample.chain <- function(X, y, z, p, n,
                                  n.samples, burnin, thin,
                                  mvnrue, groups, XtX, Xty,
                                  model, prior, t.dof,
                                  PrecomputedXtX, rv.gd, SMN, MH,
                                  pos_con = NULL, neg_con = NULL)
{
  ...
  while (k < n.samples)
  {
    ...
    if (SMN)
    {
      ...
      # Include the group-level effect in the local variances.
      lambda2_eff <- lambda2 * delta2[groups]

      if (is.null(pos_con) && is.null(neg_con))
      {
        # Original unconstrained update.
        bs  = bayesreg.sample_beta(
          X, z, mvnrue, b0, sigma2, tau2,
          lambda2_eff, omega2,
          XtX, Xty, model, PrecomputedXtX
        )
        muB = bs$m
        b   = bs$x
      }
      ...
      {
        # Truncated multivariate-normal update.

        # Compute the posterior mean and covariance.
        alpha      <- as.numeric(z - b0)
        omega2_vec <- as.numeric(omega2)

        w  <- 1 / sqrt(omega2_vec * sigma2)
        Xw <- X * w

        # Bound very small prior variances away from zero before inversion.
        Lambda_val <- sigma2 * tau2 * as.numeric(lambda2_eff)
        Lambda_val[Lambda_val < 1e-12] <- 1e-12

        Q <- crossprod(Xw)
        diag(Q) <- diag(Q) + (1 / Lambda_val)

        cholQ  <- chol(Q)
        SigmaB <- chol2inv(cholQ)

        rhs <- crossprod(Xw, alpha * w)
        muB <- as.vector(SigmaB %*% rhs)

        # Define the requested one-sided bounds.
        lb <- rep(-Inf, p)
        ub <- rep( Inf, p)
        if (!is.null(pos_con)) lb[pos_con] <- 0
        if (!is.null(neg_con)) ub[neg_con] <- 0

        b_draw <- TruncatedNormal::rtmvnorm(
          n     = 1,
          mu    = muB,
          sigma = SigmaB,
          lb    = lb,
        ...
        # Retain the column-vector shape used by the rest of bayesreg.
        b <- matrix(as.numeric(b_draw), nrow = p, ncol = 1)
      }
      ...
      # ===================================================================
      # Sample the noise scale parameter sigma2 (non-logistic only)
      mu.sigma2 <- NA
      if (model != 'logistic')
      {
        shape  <- (n + p) / 2
        scale  <- sum((e^2 / omega2) / 2) +
          sum((b^2 / delta2[groups] / lambda2 / tau2) / 2)
